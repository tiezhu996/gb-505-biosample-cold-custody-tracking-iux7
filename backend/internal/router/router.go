...
func Build(db *gorm.DB, redisClient *redis.Client, objectStore *minio.Client, cfg config.Config) (*gin.Engine, error) {
	...
	stocktakeService := service.NewStocktakeService(stocktakeRepo, auditService)
	...
	secured.GET("/stocktakes/:id/items", stocktakeHandler.ListItems)
	secured.POST("/stocktakes", middleware.RequirePermission("stocktake:execute"), stocktakeHandler.Create)
	secured.POST("/stocktakes/:id/close", middleware.RequirePermission("stocktake:execute"), stocktakeHandler.Close)
	secured.POST("/stocktakes/:id/cancel", middleware.RequirePermission("stocktake:execute"), stocktakeHandler.Cancel)
	secured.POST("/stocktake-items/:id/mark", middleware.RequirePermission("stocktake:execute"), stocktakeHandler.MarkItem)
