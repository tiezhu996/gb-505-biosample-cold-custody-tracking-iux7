...
func (h *StocktakeHandler) MarkItem(c *gin.Context) {
	...
	if err := c.ShouldBindUri(&uri); err != nil || uri.ItemID == 0 {
		util.RespondError(c, util.BadRequest("无效的盘点条目 ID"))
		return
	}
	...
	item, err := h.service.MarkItem(c.Request.Context(), ActorFromContext(c), id, uri.ItemID, input)
	if err != nil {
		util.RespondError(c, err)
		return
	}
...
func (h *StocktakeHandler) Cancel(c *gin.Context) {
	...
	item, err := h.service.Cancel(c.Request.Context(), ActorFromContext(c), id, input.Reason)
	if err != nil {
		util.RespondError(c, err)
		return
	}
